...
export function ProcessTunnelModal({
  ...
  useEffect(() => {
    if (existingTunnel) {
      ...
      setMode('create');
      // Subdomain will be auto-generated by server (no need to set it)
      setSubdomain('');
    }
  }, [existingTunnel, isOpen]);
  ...
  const handleCreate = async () => {
    ...
    try {
      const tunnel = await invoke<ProcessTunnel>('create_process_tunnel', {
        ...
        processId,
        gridId,
        subdomain: '', // Empty = auto-generate fun name
      });

      toast.success(`Public tunnel created: ${tunnel.subdomain}.localshare.tech`);
  ...
  const protocol = getProtocol();
  // In create mode, no subdomain validation needed (auto-generated)
  // In edit mode, require valid subdomain
  const isFormValid = mode === 'create'
    ? !!token
    : subdomain && isAvailable && !validationError && !isValidating && !!token;
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-6">
          ...
          {/* Ephemeral Tunnel Info (only in create mode) */}
          {mode === 'create' && (
            <div className="rounded-lg border border-purple-500/20 bg-purple-500/10 p-4">
              <div className="flex gap-2 text-xs text-purple-200/80">
                <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="font-medium mb-1">⚡ Auto-Generated Tunnel</p>
                  <p>
                    Your tunnel will get a fun auto-generated name like{' '}
                    <span className="font-mono text-purple-300">purple-dragon-7824</span>.{' '}
                    <strong>Tunnels are temporary</strong> and will stop working when you close the app.
                  </p>
                </div>
              </div>
            </div>
          ...
          {mode === 'create' && (
            <div className="rounded-lg border bg-muted p-4">
              ...
              <code className="text-sm text-blue-300 font-mono break-all">
                {protocol === 'https' ? 'https://' : ''}<span className="text-purple-400">[auto-generated]</span>.localshare.tech
              </code>
            </div>
          )}

          {/* Subdomain Input (only in edit mode) */}
          {mode === 'edit' && (
